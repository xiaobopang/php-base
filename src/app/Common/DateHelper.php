...
class DateHelper
{
     ...
     * 计算两个日期之间的天数
     */
    public static function diffDays($firstDate = "", $secondDate = "")
    {
     ...
     * @return array 当前周数组
     */
    public static function getWeek($currentDay = '')
    {
        $time = strtotime($currentDay) ?: time();
        //获取当前周几
        $week = date('w', $time);
        //星期日排到末位
        if (empty($week)) {
        ...
        $date = [];
        for ($i = 0; $i < 7; $i++) {
            $tempDate = strtotime('+' . ($i + 1 - $week) . ' days', $time);
            $date[$i] = date('Y-m-d', $tempDate);
        }
        return $date;
    }
}
