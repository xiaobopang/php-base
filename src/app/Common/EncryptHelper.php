...
class EncryptHelper
{
    ...
    public static function authcode(string $data, string $key, bool $encode = true, int $expiry = 0): array
    {
        if ('' == $data) {
            return ['', 0];
        } elseif (!$encode && strlen($data) < Constant::DYNAMIC_KEY_LEN) {
            return ['', 0];
        }
        ...
        // 密钥c用于变化生成的密文
        $keyc      = $encode ? substr(md5(microtime()), -Constant::DYNAMIC_KEY_LEN) : substr($data, 0, Constant::DYNAMIC_KEY_LEN);
        $keyd      = md5($keya . $keyc);
        $cryptkey  = $keya . $keyd;
        ...
        if ($encode) {
            ...
            $data = self::base64UrlDecode(substr($data, Constant::DYNAMIC_KEY_LEN));
        }
        ...
        if ($encode) {
            ...
            if (strlen($res) > 26) {
                ...
                if ((0 == $expTime || $expTime - $now > 0) && substr($res, 10, 16) == substr(md5(substr($res, 26) . $keyb), 0, 16)) {
     ...
     * @return string
     */
    public static function easyEncrypt(string $data, string $key = "NAO"): string
    {
        if ('' == $data) {
            return '';
        }
        ...
        return substr($key, 0, Constant::DYNAMIC_KEY_LEN) . self::base64UrlEncode($str);
    }
     ...
     * @return string
     */
    public static function easyDecrypt(string $data, string $key = "NAO"): string
    {
        if (strlen($data) < Constant::DYNAMIC_KEY_LEN) {
            return '';
        }

        $key = md5($key);
        if (substr($key, 0, Constant::DYNAMIC_KEY_LEN) != substr($data, 0, Constant::DYNAMIC_KEY_LEN)) {
            return '';
        }
